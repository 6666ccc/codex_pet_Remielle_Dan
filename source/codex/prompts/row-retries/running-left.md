Create Codex pet row `running-left` for `Remielle_Dan`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: 严格保持参考板中的同一个人形角色：粉色短发、深蓝线稿、浅蓝星形发饰、白色小翅膀、粉黄螺旋眼睛、白色服装、怀抱白色设备；不得动物化，不得改变年龄感、脸型、发型、服装、配色或画风。只允许为 Codex 标准动作和注视方向绘制同一角色的新姿态。. Preserve silhouette, face, palette, material, proportions, markings, and props.
