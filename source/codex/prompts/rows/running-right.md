Create one horizontal animation strip for Codex pet `Remielle_Dan`, state `running-right`.
...
Identity: same pet in every frame: 严格保持参考板中的同一个人形角色：粉色短发、深蓝线稿、浅蓝星形发饰、白色小翅膀、粉黄螺旋眼睛、白色服装、怀抱白色设备；不得动物化，不得改变年龄感、脸型、发型、服装、配色或画风。只允许为 Codex 标准动作和注视方向绘制同一角色的新姿态。. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `sticker`: Polished sticker mascot with bold clean shapes, crisp outline, flat colors, and minimal highlight detail. User style notes: 完全沿用原 GIF 的日系 Q 版贴纸风、线宽、配色与比例；角色身份忠实度优先。.
